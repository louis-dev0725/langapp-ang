...
class m200323_124344_create_junction_table_for_content_and_category_tables extends Migration {
    ...
    public function safeUp () {
        ...
        $this->createIndex('{{%idx-content_category-content_id}}', '{{%content_category}}',
            'content_id');

        // add foreign key for table `{{%content}}`
        $this->addForeignKey('{{%fk-content_category-content_id}}', '{{%content_category}}',
            'content_id', '{{%content}}', 'id', 'CASCADE', 'NO ACTION');

        // creates index for column `category_content_id`
        $this->createIndex('{{%idx-content_category-category_id}}', '{{%content_category}}',
            'category_id');
