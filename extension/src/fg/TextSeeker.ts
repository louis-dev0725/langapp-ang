export class TextSeeker {
    ...
    private _newlines;
    private _lineHasWhitespace;
    private _lineHasContent;
    private _forcePreserveWhitespace;
    private _generateLayoutContent;
    ...
    seek(length) {
        ...
        while (node !== null) {
            ...
            if (nodeType === TEXT_NODE) {
                ...
                lastNode = node;
                this._offset = 0;
                [enterable, newlines] = TextSeeker.getElementSeekInfo(node);
    ...
    _seekTextNodeForward(textNode, resetOffset) {
        ...
        const [preserveNewlines, preserveWhitespace] = (
            this._forcePreserveWhitespace ?
                [true, true] :
                TextSeeker.getWhitespaceSettings(textNode)
        );
     ...
     */
    static getElementSeekInfo(element): [boolean, number] {
        let enterable = true;
    ...
    static isStyleVisible(style) {
        return !(
            ...
            (
                !TextSeeker.isStyleSelectable(style) &&
                (
                    TextSeeker.isCSSColorTransparent(style.color) ||
                    TextSeeker.isCSSColorTransparent(style.webkitTextFillColor)
                )
            )
        );
    }
