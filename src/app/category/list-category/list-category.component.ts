...
export class ListCategoryComponent implements OnInit, OnDestroy {
  ...
  constructor(private api: ApiService, private router: Router, private translatingService: TranslatingService,
    private snackBar: MatSnackBar) { }

  ngOnInit() {
    this.api.getAllCategories().pipe(untilDestroyed(this)).subscribe(res => {
  ...
  handlePage(event) {
    ...
    this.api.getAllCategories(href).pipe(untilDestroyed(this)).subscribe(res => {
      this.categories = res;
      this._isLoaded = true;
    });
  }
  ...
  deleteCategory(event) {
    ...
    this.api.deleteCategory(event).pipe(untilDestroyed(this)).subscribe(res => {
      if (!(res instanceof ApiError)) {
